...
# SPECIFICATION: using association rule mining to find the strong rules
...
for index, row in df.iterrows():
    ...
    labels = {'Bread': False, 'Wine': False, 'Eggs': False, 'Meat': False, 'Cheese': False, 'Pencil': False, 'Diaper': False}
    
    labels_set = set(labels)
    
    in_transaction = row_set.intersection(labels_set)
    for label in in_transaction:
        labels[label] = True
...
#-->add your python code below
    ...
    for i in range(len(encoded_vals)):
        if encoded_vals[i][list(row['consequents'])[0]]:
            count += 1
    
    prior = count / len(encoded_vals)
